...
if (isset($_POST['TAMBAHIMUNISASI'])) {
    $result = tambahDataImunisasi($_POST);
    if ($result == 200) {
        echo "<script>alert('Berhasil menambahkan data imunisasi!'); window.location.href = 'imunisasi.php';</script>";
    } else {
        echo "<script>alert('Gagal menambahkan data imunisasi! Terjadi kesalahan.'); window.location.href = 'imunisasiTambah.php';</script>";
    }
}
    ...
    <div class="page-flex">
        ...
        <div class="main-wrapper">
            ...
            <main class="main users chart-page" id="skip-target">
                <div class="container">
                    <h2 class="main-title">Tambah Data Imunisasi</h2>
                    ...
                    <article class="sign-up">
                        <form class="sign-up-form form" action="" method="post">
                            ...
                            <label class="form-label-wrapper">
                                <p class="form-label">Imunisasi</p>
                                <select name="imunisasi_id" class="form-input" required>
                                    <?php
                                    foreach ($dataImunisasi as $imunisasi) {
                                    ?>
                                        <option value="<?= $imunisasi['id'] ?>"><?= $imunisasi['nama'] ?></option>
                                    <?php } ?>
                                </select>
                            </label>
                            <label class="form-label-wrapper">
                                <p class="form-label">Vitamin A</p>
                                <input class="form-input" type="radio" name="vitamin_a" value="Merah" checked> Merah
                                <input class="form-input" type="radio" name="vitamin_a" value="Biru"> Biru
                            </label>
                            <br><br>
                            <label class="form-label-wrapper">
                                <p class="form-label">Keterangan</p>
                                <textarea class="form-input" name="keterangan" required></textarea>
                            </label>
                            <br>
                            <button type="submit" name="TAMBAHIMUNISASI" class="form-btn primary-default-btn transparent-btn">Tambah Data Imunisasi</button>
                        </form>
                    </article>
                </div>
            </main>
